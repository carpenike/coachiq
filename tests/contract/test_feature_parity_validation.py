"""
Tests for Domain API v2 / legacy API parity.

This module is intentionally a skip stub.

The previous test body validated that ``/api/v2/...`` endpoints behaved
identically to their legacy ``/api/...`` counterparts during the v1 -> v2
migration. The v2 API has now landed in production (see
...
- v2 routes are alive (e.g. ``/api/v2/entities`` resolves in
...
pytest.skip(
    "Feature-parity validation between /api/ and /api/v2/ is obsolete: "
