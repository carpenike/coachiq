"""
Entity Service V2 - Repository Pattern Implementation

This is the migrated version of EntityService that uses repositories directly
...
from backend.repositories import EntityStateRepository, RVCConfigRepository
from backend.websocket.handlers import WebSocketManager

logger = logging.getLogger(__name__)


class EntityServiceV2:
    """
    ...
    def __init__(
        ...
        logger.info("EntityServiceV2 initialized with repositories")
    ...
    async def get_unmapped_entries(self) -> dict[str, UnmappedEntryModel]:
        ...
        """
        # Use repository for unmapped entries
        result = {}
        for key, entry in self._entity_state_repo.get_unmapped_entries().items():
    ...
    async def get_unknown_pgns(self) -> dict[str, UnknownPGNEntry]:
        ...
        """
        result = {}
        for key, entry in self._rvc_config_repo.get_unknown_pgns().items():
            entry = {
    ...
    async def control_light(self, entity_id: str, cmd: ControlCommand) -> ControlEntityResponse:
        ...
        if cmd.command == "set":
            if cmd.state == "on":
                ...
                self._entity_state_repo.set_last_known_brightness(entity_id, int(target_brightness_ui))
                new_state = True
                ...
                if current_on:
                    self._entity_state_repo.set_last_known_brightness(entity_id, int(current_brightness_ui))
...
def create_entity_service_v2() -> EntityServiceV2:
    """
    Factory function for creating EntityServiceV2 with dependencies.
