...
down_revision: Union[str, None] = 'pin_auth_20250613'
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('timestamp', sa.Float(), nullable=False, comment='Event timestamp (Unix timestamp)'),
        sa.Column('timestamp_ms', sa.BigInteger(), nullable=False, comment='Event timestamp in milliseconds'),
        sa.Column('source_component', sa.String(length=100), nullable=False, comment='Component that generated the event'),
        sa.Column('event_type', sa.String(length=100), nullable=False, comment='Type of security event'),
        sa.Column('severity', sa.String(length=20), nullable=False, comment='Event severity level'),
        sa.Column('title', sa.String(length=500), nullable=False, comment='Human-readable event title'),
        ...
        sa.Column('event_metadata', sa.JSON(), nullable=True, comment='Additional context and metadata'),
        sa.Column('incident_id', sa.String(length=255), nullable=True, comment='Associated incident ID'),
        sa.Column('correlation_id', sa.String(length=255), nullable=True, comment='Event correlation ID'),
        sa.Column('acknowledged', sa.Boolean(), nullable=False, server_default='false', comment='Whether event has been acknowledged'),
        sa.Column('acknowledged_by', sa.String(length=255), nullable=True, comment='User who acknowledged event'),
    ...
    op.create_index('idx_security_events_component_timestamp', 'security_events', ['source_component', 'timestamp'])
    ...
    op.create_table(
        'security_incidents',
        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False, comment='Primary key'),
        sa.Column('incident_id', sa.String(length=255), nullable=False, comment='Unique incident identifier'),
        sa.Column('incident_uuid', sa.String(length=36), nullable=False, comment='UUID for incident'),
        ...
        sa.Column('title', sa.String(length=500), nullable=False, comment='Human-readable incident title'),
        sa.Column('description', sa.Text(), nullable=False, comment='Detailed incident description'),
        sa.Column('status', sa.String(length=50), nullable=False, server_default='open', comment='Incident status'),
        ...
        sa.Column('assigned_to', sa.String(length=255), nullable=True, comment='Assigned investigator'),
        sa.Column('investigation_notes', sa.Text(), nullable=True, comment='Investigation notes'),
        sa.Column('event_count', sa.Integer(), nullable=False, server_default='0', comment='Number of associated events'),
        sa.Column('primary_event_id', sa.String(length=255), nullable=True, comment='Primary triggering event'),
    ...
    op.create_index('idx_security_incidents_status_severity', 'security_incidents', ['status', 'severity'])
    op.create_index('idx_security_incidents_primary_event', 'security_incidents', ['primary_event_id'])
