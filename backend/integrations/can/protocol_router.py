...
# ruff: noqa: ARG002, C901, G004, PLR0912
...
class ProtocolRouter:
    ...
    async def route_frame(self, frame: CANFrame) -> ProcessedMessage | None:
        ...
        try:
            ...
            normalized_pgn = BAMHandler.normalize_transport_pgn(frame.pgn)
            if normalized_pgn in self.TRANSPORT_PGNS:
                # Multi-packet transport protocol
                completed_message = self.bam_handler.process_frame(
                    normalized_pgn, frame.data, frame.source_address
                )
    ...
    async def _execute_safety_command(self, command: Any) -> None:
