...
class RVCConfiguration(BaseModel):
    ...
    dgn_dict: Dict[int, Dict[str, Any]] = Field(
        ...
        description="Dictionary mapping DGNs to specification entries"
    )

    spec_meta: RVCSpecMeta = Field(
        default_factory=RVCSpecMeta,
        description="Metadata from the RVC spec"
    )
    ...
    entity_map: Dict[Tuple[str, str], Dict[str, Any]] = Field(
        default_factory=dict,
        description="Dictionary mapping entity IDs to device entries"
    )

    entity_ids: Set[str] = Field(
        default_factory=set,
        description="Set of all entity IDs for validation"
    )
    ...
    unique_instances: Dict[str, Dict[str, Dict[str, Any]]] = Field(
        default_factory=dict,
        description="Dictionary of DGN instances with only one device"
    )

    # Lookup tables
    pgn_hex_to_name_map: Dict[str, str] = Field(
        default_factory=dict,
        description="Dictionary mapping DGN hex strings to PGN names"
    )

    dgn_pairs: Dict[str, str] = Field(
        default_factory=dict,
        description="Dictionary mapping command DGNs to status DGNs"
    )

    # Coach information
    coach_info: CoachInfo = Field(
        default_factory=CoachInfo,
        description="Coach metadata extracted from mapping file"
    )
    ...
    def get_entity_config(self, entity_id: str) -> RVCEntityMapping | None:
        ...
        if mapping_data:
            return RVCEntityMapping(
                dgn_hex=mapping_data["dgn_hex"],
                instance=mapping_data["instance"]
            )
        return None
