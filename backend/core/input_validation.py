...
def validate_url(url: str, allowed_schemes: list[str] = ["http", "https"]) -> str:
    ...
    try:
        ...
        # Prevent SSRF by blocking local addresses
        try:
...
class SafetyOperationRequest(BaseModel):
    ...
    entity_id: str = Field(..., regex=r"^[a-zA-Z0-9_-]{1,64}$")
    operation: str = Field(..., regex=r"^[a-zA-Z0-9_-]+$")
    pin: str | None = Field(None, regex=r"^\d{4,8}$")
